...
def generate_plot(cohort_size, protocol):
    ...
    df_various, _ = get_data(cohort_size, "various")
    ...
    plot.quad(legend_label="1.25 ≤ beta ≤ 3.0", **get_quad_args(1.24, 1.26, df_various, protocol, 0, 7))
    plot.quad(**get_quad_args(1.49, 1.51, df_various, protocol, 7, 14))
    plot.quad(**get_quad_args(1.74, 1.76, df_various, protocol, 14, 21))
    plot.quad(**get_quad_args(1.99, 2.01, df_various, protocol, 21, 28))
    plot.quad(**get_quad_args(2.24, 2.26, df_various, protocol, 28, 35))
    plot.quad(**get_quad_args(2.49, 2.51, df_various, protocol, 35, 42))
    plot.quad(**get_quad_args(2.74, 2.76, df_various, protocol, 42, 49))
    plot.quad(**get_quad_args(2.99, 3.01, df_various, protocol, 49, 56))
